...
const config = testWorkflowConfig({ features: { mirror: "shadow" }, agentLabels: { unknown: "Agent: unknown" } });
...
describe("mirror plan", () => {
  test("one done primary and one working primary does not advance", () => {
    ...
    expect(planMirror(db, config, [done, working]).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
    db.close();
  });
  ...
  test("captain hand drag without a newer file signal is reported, not repaired", () => {
    const root = mkdtempSync("/private/tmp/fml-plan-"); roots.push(root);
    const db = new StateDatabase(join(root, "db"), join(root, "backups"));
    db.snapshot({ issue: "ABC-1", role: null, assignee: "Captain", labels: [], agent_label: null, last_actor: "Captain", last_signal: null, observed_at: "2026-01-01T00:02:00Z" });
  ...
  test("support PR transitions do not drive issue state", () => {
    ...
    db.observe(green);
    expect(planMirror(db, config, [green]).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
    db.close();
  });
  ...
  test("signals from torn-down tasks no longer affect reduction", () => {
    ...
    expect(planMirror(db, config, [working]).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
    db.close();
  });
  ...
  test("a relinked task does not inherit signals from its prior lifecycle", () => {
    ...
    expect(planMirror(db, config, []).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
    db.close();
  });

  test("a same-second role change does not reclassify prior lifecycle signals", () => {
    const root = mkdtempSync("/private/tmp/fml-plan-"); roots.push(root);
    const db = new StateDatabase(join(root, "db"), join(root, "backups"));
    db.snapshot({ issue: "ABC-1", role: "building", assignee: "Firstmate", labels: [], agent_label: null, last_actor: "Firstmate", last_signal: null, observed_at: "2026-01-01T00:00:00Z" });
    db.linkTask({ task: "worker", issue: "ABC-1", role: "support", worktree: null, harness: null, spawned_at: "2026-01-01T00:00:00Z", torn_down_at: null });
    db.observe({ id: "support-done", source: "status", task: "worker", issue: "ABC-1", verb: "done", key: "default", note: null, observed_at: "2026-01-01T00:01:00Z" });
    db.linkTask({ task: "worker", issue: "ABC-1", role: "primary", worktree: null, harness: null, spawned_at: "2026-01-01T00:01:00Z", torn_down_at: null });
  ...
  test("support control signals never drive issue state", () => {
    for (const verb of ["dispatch", "dispatch-scout", "lane-cap"]) {
      const root = mkdtempSync("/private/tmp/fml-plan-"); roots.push(root);
      const db = new StateDatabase(join(root, "db"), join(root, "backups"));
      db.snapshot({ issue: "ABC-1", role: null, assignee: "Firstmate", labels: [], agent_label: null, last_actor: "Firstmate", last_signal: null, observed_at: "2026-01-01T00:00:00Z" });
      db.linkTask({ task: "support", issue: "ABC-1", role: "support", worktree: null, harness: null, spawned_at: "2026-01-01T00:00:00Z", torn_down_at: null });
      ...
      db.observe(observation);
      expect(planMirror(db, config, [observation]).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
      db.close();
    }
  });
  ...
  test("an active green primary PR preserves the approval gate", () => {
    const root = mkdtempSync("/private/tmp/fml-plan-"); roots.push(root);
    const db = new StateDatabase(join(root, "db"), join(root, "backups"));
    db.snapshot({ issue: "ABC-1", role: "review-gate", assignee: "Captain", labels: [], agent_label: null, last_actor: "Firstmate", last_signal: null, observed_at: "2026-01-01T00:00:00Z" });
  ...
  test("a completed primary task advances the issue to done", () => {
    ...
    db.observe(done);
    const state = planMirror(db, config, [done]).actions.find((action) => action.job.kind === "linear.issue-role");
    expect(state?.job.payload).toMatchObject({ role: "done" });
    db.close();
  });
  ...
  test("a completed mirror job is revived when drift returns", () => {
    ...
    const job = db.jobs().find((item) => item.kind === "linear.issue-role")!;
    db.finishJob(job.id, "issue-id", "2026-01-01T00:02:00Z");
    db.snapshot({ issue: "ABC-1", role: "building", assignee: "Firstmate", labels: [], agent_label: null, last_actor: "service", last_signal: null, observed_at: "2026-01-01T00:03:00Z" });
  ...
  test("one primary PR cannot bypass unfinished primary tasks", () => {
    for (const verb of ["pr-green", "pr-merged"]) {
      ...
      expect(planMirror(db, config, [pr, working]).actions.filter((action) => action.job.kind === "linear.issue-role")).toHaveLength(0);
      db.close();
    }
  });
  ...
  test("lane capacity counts only the dispatched issue team", () => {
    ...
    multiTeam.teams.push({ key: "XYZ", projects: [], managed: "all", roles: { ...FULL_ROLES }, agent_labels: { unknown: "Agent: unknown" } });
    db.snapshot({ issue: "ABC-1", role: null, assignee: "Firstmate", labels: [], agent_label: null, last_actor: "Firstmate", last_signal: null, observed_at: "2026-01-01T00:00:00Z" });
    for (let index = 1; index <= 8; index += 1) {
      db.snapshot({ issue: `XYZ-${index}`, role: "building", assignee: "Firstmate", labels: [], agent_label: null, last_actor: "Firstmate", last_signal: null, observed_at: "2026-01-01T00:00:00Z" });
    }
    db.linkTask({ task: "primary", issue: "ABC-1", role: "primary", worktree: null, harness: null, spawned_at: "2026-01-01T00:00:00Z", torn_down_at: null });
    ...
    db.observe(dispatch);
    const state = planMirror(db, multiTeam, [dispatch]).actions.find((action) => action.job.kind === "linear.issue-role");
    expect(state?.job.payload).toMatchObject({ role: "building" });
    db.close();
  });
  ...
  test("blocking primary signals outrank later control verbs", () => {
    for (const verb of ["dispatch", "dispatch-scout", "lane-cap"]) {
      ...
      const state = planMirror(db, config, [control]).actions.find((action) => action.job.kind === "linear.issue-role");
      expect(state?.job.payload).toMatchObject({ role: "decision-firstmate" });
      db.close();
    }
  });
});
