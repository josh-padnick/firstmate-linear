...
function refuseHeldRoute(
  installation: FirstmateInstallation,
  store: AdapterStore,
  route: FirstmateHomeRoute,
) {
  const observation = new FleetStore(store, installation.homeId)
    .observations()
    .find((h) => h.owner.homeId === route.owner.homeId);
...
export async function checkRoutedBrief(
  installation: FirstmateInstallation,
  store: AdapterStore,
  route: FirstmateHomeRoute,
  ...
  refuseHeldRoute(installation, store, route);
  const receipt = BriefReceipt.parse(inputReceipt);
  const attempt = AttemptRef.parse(inputAttempt);
  if (
    receipt.task.homeId !== route.owner.homeId ||
    attempt.task.homeId !== route.owner.homeId ||
    receipt.task.taskId !== attempt.task.taskId
  )
    throw new FmError(
      "firstmate.scope_mismatch",
      "The instruction evidence must name the same task and owning home.",
    );
  const expected = store.get<{ revision: string | null }>(
    installation.homeId,
    "fleet",
    "registry-revision",
  );
  ...
  if (!expected?.revision || expected.revision !== revision)
    throw new FmError("firstmate.route_changed", "The registered route needs a fresh fleet check.");
  const base = { attempt, observedAt: new Date().toISOString(), launchRevision: null };
  if (route.kind === "local") {
    const result = await checkBrief(
      { ...installation, home: route.home, homeId: route.owner.homeId },
      receipt,
      attempt,
    );
  ...
  if (
    after === null ||
    generation(after) !== attempt.attemptId ||
...
export async function readRoutedTask(
  installation: FirstmateInstallation,
  store: AdapterStore,
  route: FirstmateHomeRoute,
  ...
  if (route.kind === "local") {
    ...
    const file = async (path: string) => {
      const response = await reader.command(route, "fm-remote-file.sh", ["get", path, "262144"]);
